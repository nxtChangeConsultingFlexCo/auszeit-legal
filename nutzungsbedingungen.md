---
title: Nutzungsbedingungen
permalink: /nutzungsbedingungen/
---

# Nutzungsbedingungen für Auszeit

Stand: 22. September 2026

## 1. Wer wir sind

Auszeit ist eine App für iPhone von

nxtChange Consulting FlexCo
Neugasse 9/1
8045 Graz, Österreich
Firmenbuchnummer: FN 648962g, Landesgericht für ZRS Graz
UID: ATU81922038
Ansprechpartner: Karl Maier
karl.maier@nxtchange-consulting.com

(im Folgenden „wir“). Fragen zur App beantworten wir unter der genannten E-Mail-Adresse.

## 2. Was die App tut

Auszeit hält ausgewählte Apps zu Zeiten, die du festlegst, nicht sofort auf. Öffnest du eine
solche App in diesem Zeitfenster, zeigt dein iPhone zuerst eine Pause von 90 Sekunden. Danach
entscheidest du: die App liegen lassen oder trotzdem öffnen. Alle anderen Apps bleiben unberührt.

Die App nutzt dafür die Bildschirmzeit-Funktionen von iOS („Family Controls“). Sie kann nur
wirken, wenn du ihr den Zugriff auf die Bildschirmzeit erlaubst.

## 3. Was die App nicht verspricht

Auszeit ist ein Hilfsmittel, keine Sperre. Du kannst jede Pause übergehen, den Bildschirmzeit-Zugriff
in den iOS-Einstellungen entziehen oder die App löschen. Damit endet jede Auszeit. Wir versprechen
keinen bestimmten Erfolg, etwa weniger Bildschirmzeit.

iOS setzt technische Grenzen, die wir nicht beeinflussen können. Zum Beispiel bleibt eine App nach
„Trotzdem öffnen“ für 15 Minuten ohne Pause nutzbar, und iOS kann eine begrenzte Zahl von Apps
gleichzeitig pausieren.

## 4. Gratis und Plus

Kostenlos enthält Auszeit eine Auszeit mit bis zu zwei Apps.

„Auszeit Plus“ ist ein Abonnement und erlaubt weitere Auszeiten sowie mehr als zwei Apps je
Auszeit. Es gibt Plus als Jahres- und als Monatsabo. Die Preise stehen in der App und im App Store.

Für den Kauf gilt:

- Der Kauf läuft über deinen Apple-Account. Vertragspartner für die Zahlung ist Apple.
- Das Abo verlängert sich automatisch, wenn du es nicht mindestens 24 Stunden vor Ablauf kündigst.
  Der Betrag wird innerhalb der letzten 24 Stunden vor Ablauf über deinen Apple-Account abgebucht.
- Kündigen kannst du jederzeit: in der App über „Auszeit Plus“ oder unter iPhone-Einstellungen ›
  Apple-Account › Abos. Bis zum Ende des bezahlten Zeitraums bleibt Plus aktiv.
- Erstattungen entscheidet Apple nach seinen Regeln. Anträge stellst du bei Apple unter
  reportaproblem.apple.com.
- Endet Plus, gilt wieder: eine Auszeit, zwei Apps. Deine erste Auszeit bleibt erhalten; weitere
  Einträge bleiben gespeichert, greifen aber nicht mehr.

Es gilt außerdem Apples Standard-Endbenutzer-Lizenzvertrag für Apps aus dem App Store
(„Licensed Application End User License Agreement“), soweit diese Bedingungen nichts anderes regeln:
https://www.apple.com/legal/internet-services/itunes/dev/stdeula/

## 5. Nutzungsrecht

Du erhältst ein einfaches, nicht übertragbares Recht, die App auf Apple-Geräten zu nutzen, die
mit deinem Apple-Account verbunden sind. Du darfst die App nicht verändern, nachbauen oder
weiterverkaufen.

## 6. Haftung

Wir haften unbeschränkt für Vorsatz und grobe Fahrlässigkeit sowie für Schäden an Leben, Körper und
Gesundheit. Für leichte Fahrlässigkeit haften wir nur bei Verletzung wesentlicher Vertragspflichten
und begrenzt auf den vorhersehbaren, vertragstypischen Schaden. Zwingende Rechte von Verbrauchern
bleiben unberührt.

## 7. Änderungen

Wir können diese Bedingungen ändern, etwa wenn sich die App oder das Recht ändert. Die aktuelle
Fassung ist immer unter dieser Adresse abrufbar. Wesentliche Änderungen kündigen wir in der App
oder im App Store an.

## 8. Recht und Gerichtsstand

Es gilt österreichisches Recht unter Ausschluss des UN-Kaufrechts. Für Verbraucher aus dem
EU-Ausland gelten daneben die zwingenden Verbraucherschutzvorschriften ihres Wohnsitzstaates.

Die Europäische Kommission stellt eine Plattform zur Online-Streitbeilegung bereit:
https://ec.europa.eu/consumers/odr. Wir sind nicht verpflichtet und nicht bereit, an
Streitbeilegungsverfahren vor einer Verbraucherschlichtungsstelle teilzunehmen.
